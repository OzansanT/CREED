...
import { bindSidebarToggle, bindSidebarMenu } from "./sidebar-input.js";
...
function factoryReset() { const confirmed = confirm("Factory reset will restore the default pan, zoom, sidebar, card positions and remove the saved anchor. Continue?"); if (!confirmed) return; clearStoredState(); state.anchor = null; state.sidebarCollapsed = false; state.sidebarView = "canvas"; state.originCard = { worldX: 0, worldY: 0 }; state.jsonCard = { visible: false, worldX: 0, worldY: 0 }; returnToOrigin({ state, canvas: elements.canvas, update, persist }); showToast(elements.toast, "Factory settings restored"); }
...
bindSidebarToggle({ button: elements.sidebarToggle, state, update, persist });
bindSidebarMenu({ canvasButton: elements.canvasMenuBtn, componentsButton: elements.componentsMenuBtn, addJsonCardButton: elements.addJsonCardBtn, canvas: elements.canvas, jsonCard: elements.jsonComponentCard, state, update, persist });
